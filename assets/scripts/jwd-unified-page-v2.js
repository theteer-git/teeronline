...
(() => {
  ...
  async function fetchRecent() {
    ...
    loadingRecent = (async () => {
      const response = await fetch(RECENT_URL, {cache:"no-store", referrerPolicy:"no-referrer"});
  ...
  async function refreshCommonCard() {
    try {
      const response = await fetch("/common-numbers.html", {cache:"no-store", referrerPolicy:"no-referrer"});
  ...
  async function loadPlan() {
    try {
      ...
      console.warn("JWD polling plan request failed:", error);
    }
  }
  function intervalMs() {
