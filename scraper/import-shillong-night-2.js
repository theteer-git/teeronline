...
const EXCEL_FILE = path.join(__dirname, "../Book1.CSV.xlsx");
const JSON_FILE = path.join(__dirname, "../data/all-results.json");

const GAME_ID = "SHN2";
const GAME_NAME = "Shillong Night 2";
const CITY = "Shillong";

const FR_TIME = "20:00:00";
const SR_TIME = "20:15:00";
...
try {
  ...
  const records = Array.isArray(existingData)
    ? existingData
  ...
  for (const row of rows) {
    ...
    const fr = normalizeNumber(row.FR || row["F/R"] || row.fr);
    const sr = normalizeNumber(row.SR || row["S/R"] || row.sr);
  ...
  console.log("Shillong Night 2 import completed");
